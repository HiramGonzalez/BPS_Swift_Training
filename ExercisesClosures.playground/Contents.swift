...
let reverseOrderedSurnames = surnames.sorted(by: {(name1: String, name2: String) -> Bool in
    return name1 > name2
})
...
let reverseSortedAverages = battingAverages.sorted(by: {(average1: Double, average2: Double) -> Bool in
...
for i in numbersAsStrings{
    numbersAsInt.append(Int(i)!)
}

let divisibleBy12 = numbersAsInt.filter({(number: Int) -> Bool in
    return number % 12 == 0
})
//: __3b.__
//: Rewrite the filtering closure expression to be as concise as possible.
let divisibleAndConcise = numbersAsInt.filter({
    $0 % 12 == 0
})
//: __Problem 4__
//:
...
let pricesInDollars = pricesInRupees.map({(price: Int) -> Int in
    return Int(price / 82)
})
...
let goalTimes = oldTimes.map({(time: String) -> NSString in
    return timeStringFromInterval(timeIntervalFromString(time) + 13)
})
print(goalTimes)

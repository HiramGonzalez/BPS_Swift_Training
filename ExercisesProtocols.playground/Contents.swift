...
class Teenager: Babysitter {
    ...
    func playCandyland(_ numberOfTimes: Int) {
        print("We'll play Candyland \(numberOfTimes) times.")
    }
    
    func read(_ book: String, firstLine: String, asleep: Bool) -> Bool {
        if book == "Cynderella" && firstLine == "Once upon a time"{
            return true
        } else {
            return false
        }
...
let teen = Teenager(age: 15, responsible: true, patience: .low)
print(teen.read("Three bears", firstLine: "Once upon a time", asleep: false))
//: __Problem 2__
//:
...
let animal = Animal(species: "cat", numberOfLegs: 4, size: .medium, softFur: true)
...
protocol Mover {
    ...
    func laodVan(_ empty: Bool) -> Bool
}
...
extension UIColor
{
    ...
    static func pistachio() -> UIColor {
        return UIColor(red: 147, green: 197, blue: 114, alpha: CGFloat(1.0))
    }
}

var pistachio = UIColor.pistachio()
print(pistachio)
